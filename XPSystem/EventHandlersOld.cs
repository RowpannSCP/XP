    ...
    public class EventHandlersOld
    {
        ...
        public void OnJoined(PluginAPI.Core.Player ply)
        {
            ...
            if (hub.authManager.DoNotTrack) // dnt
            {
                ...
                hub.gameConsoleTransmission.SendToClient($"[REPORTING] {Main.Instance.ConfigOld.DNTHint}", "white");
                return;
            }
        ...
        public void OnKill(PluginAPI.Core.Player player, PluginAPI.Core.Player attacker, DamageHandlerBase damageHandler)
        {
            ...
            int xp = 0;
            bool attackerRoleHasDict = Main.Instance.ConfigOld.KillXP.TryGetValue(attackerRoleId, out var killxpdict);
            bool victimRoleHasEntry = attackerRoleHasDict && killxpdict.TryGetValue(oldRole, out xp);
            bool usingDefaultDict = !attackerRoleHasDict && Main.Instance.ConfigOld.KillXP.TryGetValue(RoleTypeId.None, out killxpdict);
        ...
        public void OnEscape(PluginAPI.Core.Player ply, RoleTypeId role)
        {
            ...
            if (!hub.inventory.IsDisarmed())
                return;
            if (!Main.Instance.ConfigOld.EscapeXP.TryGetValue(hub.GetRoleId(), out int xp))
            {
        ...
        public void OnRoundEnd(RoundSummary.LeadingTeam team)
        {
            ...
            foreach (var hub in ReferenceHub.AllHubs)
            {
                ...
                if (log is null)
                    continue;
                log.AddXP(Main.Instance.ConfigOld.TeamWinXP, Main.GetTranslation("teamwin"));
            }
#if EXILED
            ...
            if (Main.Instance.ConfigOld.LogXPGainedMethods)
            {
        ...
        public void OnInteractingDoor(Exiled.Events.EventArgs.Player.InteractingDoorEventArgs ev)
        {
            ...
            if (!ev.IsAllowed)
                return;
            if (ev.Player.CurrentItem is null && Main.Instance.ConfigOld.DontGiveDoorXPEmptyItem)
                return;
            if (Main.Instance.ConfigOld.DoorInteractXP.ContainsKey(ev.Door.Type) && Main.Instance.ConfigOld.DoorInteractXP[ev.Door.Type] != 0)
            {
                ...
                if (!Main.Instance.ConfigOld.DoorXPOneTime || !value.Contains(ev.Door.Type))
                {
                    value.Add(ev.Door.Type);
                    var log = ev.Player.ReferenceHub.GetLog();
                    bool hid = ev.Door.Room.Type == Exiled.API.Enums.RoomType.HczHid;
                    log.AddXP(Main.Instance.ConfigOld.DoorInteractXP[ev.Door.Type], hid ? "doorhid" : Main.GetTranslation(GetDoorKey(ev.Door.Type)));
                }
            }
        }
        ...
        public void OnUpgradingItem(ReferenceHub ply, ItemCategory type)
        {
            ...
            if (ply.authManager.DoNotTrack) // dnt
                return;
            if (Main.Instance.ConfigOld.UpgradeXP.ContainsKey(type) && Main.Instance.ConfigOld.UpgradeXP[type] != 0)
            {
                ...
                if (!Main.Instance.ConfigOld.UpgradeXPOneTime || !value.Contains(type))
                {
                    value.Add(type);
                    var log = ply.GetLog();
                    log.AddXP(Main.Instance.ConfigOld.UpgradeXP[type], 
        ...
        public void Spawned(PluginAPI.Core.Player ply, RoleTypeId roleType)
        {
            ...
            {
                var log = hub.GetLog();
                if (Main.Instance.ConfigOld.SpawnXP.TryGetValue(roleType, out var value) || Main.Instance.ConfigOld.SpawnXP.TryGetValue(RoleTypeId.None, out value))
                    log.AddXP(value, Main.GetTranslation($"spawned{roleType.ToString()}"));
                else if (Main.Instance.ConfigOld.Debug)
		...
		void OnSearchedPickup(PluginAPI.Core.Player ply, ItemPickupBase pickup)
		{
            ...
            if (Main.Instance.ConfigOld.PickupXPOneTime)
            {
        ...
        {
            var log = ply.GetLog();
            if(Main.Instance.ConfigOld.PickupXP.TryGetValue(pickupBase.Info.ItemId, out var value) || Main.Instance.ConfigOld.PickupXP.TryGetValue(ItemType.None, out value))
        ...
        public void OnThrowingGrenade(Exiled.Events.EventArgs.Player.ThrownProjectileEventArgs ev)
        {
            ...
            if (Main.Paused) // pause
                return;
            var log = ev.Player.ReferenceHub.GetLog();
            if(Main.Instance.ConfigOld.ThrowXP.TryGetValue(ev.Projectile.ProjectileType, out var value))
		...
		void OnPlayerDroppedItem(PluginAPI.Core.Player ply, ItemBase item)
		{
            ...
            if (hub.authManager.DoNotTrack) // dnt
                return;
            var log = hub.GetLog();
            if (Main.Instance.ConfigOld.DropXPOneTime)
            {
                ...
                if (AlreadyGainedPlayers5.TryGetValue(hub, out var list))
                {
                    ...
                    {
                        list.Add(itemType);
                        if(Main.Instance.ConfigOld.DropXP.TryGetValue(itemType, out var value) || Main.Instance.ConfigOld.DropXP.TryGetValue(ItemType.None, out value))
                            log.AddXP(value, Main.GetTranslation($"drop{itemType.ToString()}"));
                    }
                }
                return;
            }
            if(Main.Instance.ConfigOld.DropXP.TryGetValue(itemType, out var value2) || Main.Instance.ConfigOld.DropXP.TryGetValue(ItemType.None, out value2))
		...
		void OnPlayerUsedItem(PluginAPI.Core.Player ply, ItemBase item)
		{
            ...
            if (hub.authManager.DoNotTrack) // dnt
                return;
            var log = hub.GetLog();
            if (Main.Instance.ConfigOld.UseXPOneTime)
            {
                ...
                if (AlreadyGainedPlayers6.TryGetValue(hub, out var list))
                {
                    ...
                    {
                        list.Add(itemType);
                        if(Main.Instance.ConfigOld.UseXP.TryGetValue(itemType, out var value) || Main.Instance.ConfigOld.UseXP.TryGetValue(ItemType.None, out value))
                            log.AddXP(value, Main.GetTranslation($"use{itemType.ToString()}"));
                    }
                }
                return;
            }
            if(Main.Instance.ConfigOld.UseXP.TryGetValue(itemType, out var value2) || Main.Instance.ConfigOld.UseXP.TryGetValue(ItemType.None, out value2))
